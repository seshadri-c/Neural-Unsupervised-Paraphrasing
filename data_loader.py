...
class DataGenerator(Dataset):
	...
	def __init__(self, data_1, data_2):
		self.files = self.get_files(data_1, data_2)
	...
	def get_files(self,data_1, data_2):

		print("Data Recieved.")		

		source_path_1,  gt_path_1 = data_1
		source_path_2,  gt_path_2 = data_2

		src_list = []
		tgt_list = []

		with open(source_path_1, 'rb') as handle:
			lines_1 = pickle.load(handle)

		with open(source_path_2, 'rb') as handle:
		...
		src_list.extend(lines_2)

		tgt_list.extend(lines_3)
		tgt_list.extend(lines_4)
...
def load_data(data_1, data_2, batch_size=128, num_workers=2, shuffle=True):
    
	dataset = DataGenerator(data_1, data_2)
